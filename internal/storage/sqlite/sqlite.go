...
func (s *Storage) SaveURL(urlToSave, alias string) (int64, error) {
	const op = "storage.sqlite3.SaveURL"
...
func (s *Storage) DeleteURL(alias string) error {
	...
	// check alias is exists
	err := s.db.QueryRow("SELECT 1 FROM url WHERE alias = ?", alias).Scan(new(int))
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return storage.ErrURLNotFound
		}
	...
	if err != nil {
		return fmt.Errorf("%s: delete alias: %w", op, err)
	}
